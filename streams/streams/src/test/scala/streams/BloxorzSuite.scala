...
class BloxorzSuite {
  ...
  @Test def `neighborsWithHistory equals`:Unit =
    new Level1 {
      lazy val expected = LazyList(
        (Block(Pos(1,2),Pos(1,3)), List(Right,Left,Up)),
        (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
      )
      assertEquals(expected, neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Left,Up)))
    }

  @Test def `newNeighborsOnly equals` : Unit =
    new Level1 {
      ...
      assertEquals(LazyList((Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))), newNeighbours)
    }
